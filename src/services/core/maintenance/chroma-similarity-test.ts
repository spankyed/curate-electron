...
async function testSimilarity(doc1: any) {
  ...
  let relevancy_score: number =
    top_5_scores.reduce((acc, val) => acc + val, 0) / top_5_scores.length;
  console.log('Manual \n -----------------');
...
async function testSimilarity2(doc1: any) {
  ...
  const relevancyScores = results.distances?.[0] ? results.distances?.[0] : [];
