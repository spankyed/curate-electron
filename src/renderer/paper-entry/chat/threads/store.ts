...
export const selectAndLoadMessagesAtom = atom(
  ...
  async (get, set, paperId: string, selectedThread: any) => {
    try {
      ...
      const messages = await api.getMessages(selectedThread.id)
...
export const branchThreadAtom = atom(null, async (get, set, paperId: string, message: any) => {
  ...
  try {
    const { messages, thread: newThreadRecord } = await api.branchThread(newThread)
...
export const addNewThreadAtom = atom(null, async (get, set, paperId: string) => {
  const threadOptions = get(threadOptionsAtom)
  const newThread = {
    paperId,
  ...
  try {
    const newThreadRecord = await api.createThread(newThread)
