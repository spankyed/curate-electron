...
export const fetchCalendarModelAtom = atom(null, async (get, set) => {
  try {
    set(calendarStateAtom, 'loading');
    set(lastRecordReachedAtom, false);

    const calendarModel: CalendarModel = await getCalendarModelData();
...
export const calendarLoadMoreAtom = atom(
  ...
  async (get, set, date) => {
    try {
      ...
      const calendarModel: CalendarModel = await calendarLoadMore(date);
...
export const calendarLoadMonthAtom = atom(
  ...
  async (get, set, date) => {
    ...
    try {
      const calendarModel: CalendarModel = await calendarLoadMonth(date);
      set(calendarModelAtomBase, calendarModel);
      set(calendarStateAtom, 'ready');
      // set(selectedDateAtom, dateList[0]?.dates[0]?.value ?? '');
    } catch (error) {
...
export const resetDateStatusCalenderAtom = atom(null, async (get, set, date) => {
  try {
...
export const scrapePapersAtom = atom(null, async (get, set, value) => {
  ...
  try {
    set(targetDateAtom, (prevDate) => ({
      ...prevDate,
      date: { ...prevDate.date, status: 'scraping' },
    }));

    await scrapeDate(value);
  } catch (error) {
    console.error('Scraping failed:', error);
    set(targetDateAtom, (prevDate) => ({
      ...prevDate,
      date: { ...prevDate.date, status: 'error' },
    }));
  }
});
